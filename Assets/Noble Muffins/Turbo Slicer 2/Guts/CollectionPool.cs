...
namespace NobleMuffins.TurboSlicer.Guts
{
	public class CollectionPool<TCollection, TElement> where TCollection: class, ICollection<TElement>
	{
		...
		public CollectionPool (int poolSize, Func<int, TCollection> instantiateWithCapacity)
		{
			this.poolSize = poolSize;
			this.poolTable = new TCollection[poolSize];
		...
		public DisposableBundle<TCollection> Get (int desiredCapacity)
		{
			...
			lock (key) {
				for(int i = 0; i < poolSize; i++) {
					if(!useTable[i] && !object.ReferenceEquals(poolTable[i], null) && poolTable[i].Count >= desiredCapacity) {
						o = poolTable[i];
						o.Clear();
						useTable[i] = true;
						break;
