...
namespace CaronteFX
{
  public class CNFieldController : IListController 
  {
    ...
    public void RemoveSelected()
    {
      ...
      field_.RemoveNodesAt(listNodeIdx);
    ...
    //-----------------------------------------------------------------------------------
    public void ConvertGameObjectReferencesToNameSelectors()
    {
    }
    #endregion
    //-----------------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------------
    public CommandNode[] GetCommandNodesNotBody()
    {
      List<CommandNode> listCommandNode = field_.CommandNodes;
      listCommandNodeAux_.Clear();
      ...
      foreach( CommandNode cNode in listCommandNode )
      {
        ...
        foreach( CRTreeNode treeNode in listTreeNodeAux_)
        {
          ...
          if ( !(node is CNBody) && !(node is CNGroup) )
          {
            listCommandNodeAux_.Add(node);
          }
        }
    ...
    public void FillOptionsMenu( GenericMenu optionsMenu )
    {
      if ( AreGameObjectsAllowed() )
      {
        ...
        optionsMenu.AddSeparator("");
        optionsMenu.AddItem(new GUIContent("Convert gameobject references to wildcards"), false, ConvertGameObjectReferencesToNameSelectors);
        optionsMenu.AddSeparator("");
      ...
      else
      {
        ...
        optionsMenu.AddSeparator("");
        optionsMenu.AddDisabledItem(new GUIContent("Convert to gameobject references to wildcards"));
        optionsMenu.AddSeparator("");
        optionsMenu.AddItem(new GUIContent("Clear field"), false, () => { Clear(true); } );
      }
    }
