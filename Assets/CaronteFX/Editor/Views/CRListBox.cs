...
{ 
  ...
  { 
    ...
    for ( int i = visibleBeginIdx; i < visibleEndIdx; i++ )
    {
      ...
      int itemIndentLevel = Controller.ItemIndentLevel( i );
      float labelStart = itemIndentLevel * pixelSpace_;
      ...
      DrawItem( i, itemIndentLevel, commandBox, labelBox, toggleBox, iconBox, textBox, textBoxWithoutToggle, s1, s2, isGroup );
    ...
    Controller.FinishRenderingItems();
  } 
  //----------------------------------------------------------------------------------
  private void DrawItem( int itemIdx, int itemIndentLevel, Rect commandBox, Rect labelBox, Rect toggleBox, Rect iconBox, Rect textBox, Rect textBoxWithoutToggle, string itemName, string realName, bool isGroup )
  {
        ...
        { 
          if ( itemIdx != Controller.ItemIdxEditing )
          {
            ...
            if (ev.button == 0)
            {
              if (ev.clickCount == 1)
              {
                Controller.OnClickItem(itemIdx, ev.control || ev.command, ev.shift, ev.alt, false);
              }
      ...
      case EventType.MouseUp:
        ...
        if ( commandBox.Contains( ev.mousePosition ) && itemIdx != Controller.ItemIdxEditing )
        {
          if (ev.button == 0)
          {
            if (ev.clickCount == 1)
            {
              Controller.OnClickItem(itemIdx, ev.control || ev.command, ev.shift, ev.alt, true);
            }
            else if (ev.clickCount == 2)
            {
              Controller.OnDoubleClickItem(itemIdx);
  ...
  private void ProccessOtherEvents( Event ev, Rect listBox )
  {
    ...
    case EventType.KeyDown:
      if (Focused)
      {
        ...
        {
          Controller.OnSelectItemPrev();
        }
        else if (ev.keyCode == KeyCode.DownArrow)
        {
          Controller.OnSelectItemNext();
        }
      }
      break;
    }
