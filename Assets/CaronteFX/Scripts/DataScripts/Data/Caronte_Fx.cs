...
{
  /// <summary>
  /// Data class for Physics Scene persistence, and some auxiliary drawing through OnDrawGizmos.
  /// Contains all the nodes and definitions that compose a Physics Scene.
  /// </summary>
  [AddComponentMenu("CaronteFX/CaronteFX Definition")]
  public class Caronte_Fx : MonoBehaviour
  {
    ...
    [SerializeField]
    CREffectData effectData_ = null;
    public CREffectData effect
    {
      get { return effectData_; }
      set { effectData_ = value; }
    }
    ...
    [SerializeField]
    private bool firstUse_ = true;
    public bool FirstUse
    {
      get { return firstUse_; }
      set { firstUse_ = value; }
    }
    ...
    {
      if (effectData_ == null || effectData_.rootNode_ == null)
      {
        AddEffect();
        return true;
      }
    ...
    public bool UpdateRootNodeName()
    {
      bool wasUpdated = false;
      string rootNodeName = e_getRootNode().Name;
      if (rootNodeName != gameObject.name)
      {
    ...
    public void OpenRootNodeIfNotOpened()
    {
      CNGroup rootNode = e_getRootNode();
      if (!rootNode.IsOpen)
      {
